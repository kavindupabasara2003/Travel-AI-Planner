...
const aspectMap = ref({})  // city → scores dict
...
async function fetchAspectScores(itinerary) {
  ...
  try {
    const res = await axios.get('/api/v1/aspects/', {
      params: { names: cities.join(',') }
    })
    const map = {}
    ...
    // ABSA data not available yet — silent fail
  }
}
...
<template>
  <div class="dashboard">
    ...
    <div v-else class="itinerary-content">
      <!-- Hero Header with city-specific image -->
      <div class="trip-hero">
        <div class="hero-image-wrapper">
          <img class="hero-image" :src="heroImage" :alt="getLocation(chatStore.itinerary)" />
          <!-- Weather optimised badge -->
          <div
            v-if="chatStore.itinerary.weather_optimized"
      ...
      <div class="timeline-container">
        ...
        <div class="days-list">
          <ItineraryCard
            ...
            :key="day.day"
            :day="day"
            :aspect-scores="getAspectScores(day)"
          />
        </div>
      </div>
    </div>
